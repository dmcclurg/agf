      ...
      $output = wp_list_pages('sort_column=menu_order'.$depthStr.'&title_li=&echo=0&child_of='.$rootPost->ID);
      	...
      	if (!$useRoot and $addParent and $rootPost->post_parent != 0){
    		...
    		if ($parentPosition=='top'){
    			$output = '<li><h2><a href="'.get_permalink($rootPost->post_parent).'">'.$parentIcon.'&nbsp;'.$parentTitle.'</a></h2></li>'.$output;
    		} else {
    		    $output = $output.'<li><h2><a href="'.get_permalink($rootPost->post_parent).'">'.$parentIcon.'&nbsp;'.$parentTitle.'</a></h2></li>';
           	}    		  
      	}  
      	...
      	if (!empty($output)){
    		...
    		echo "<ul class=\"sidebar-subpages\">";    		
        		echo '<h2><a href="'.get_permalink($rootPost->ID).'">'.$title.'</a></h2>'.$after_title;            		
    			echo '<ul>';
        			echo $output;
        		echo '</ul>';		
    		echo '</ul>';
